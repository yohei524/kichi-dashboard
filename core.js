...
function buildMonthGuidance(monthF) {
  ...
  if (monthF.isTenchu && mEnergy >= 10) {
    ...
    summary = '今月の従星は' + mJyusei + 'で、出力が大きく出ます。決断・前進・責任ある動きが活きる月です。気になっていたことを今月中に一歩進めるのに向いています。';
  } else if (mEnergy > 0 && mEnergy <= 2 && hasGood) {
    // 【260815修正】低出力 × 合法。従星の点数だけで「休め」と結論を出していた実装の誤り。
    //   低出力＝自分から押す力が出ない。合法＝まとまる・広がる方向。
    //   両方あるなら動きは起きる。主導権が自分側にないだけ。→「押すな、受けろ」
    summary = '今月の従星は' + mJyusei + 'で、自分から押していく力は出にくいです。ただ、まとまる方向の配置が同時に出ています。こちらから仕掛けるより、来た話を受ける形で動くほうが進みます。声がかかったら受けておくと、後につながります。';
  } else if (mEnergy > 0 && mEnergy <= 2 && hasCaution) {
    summary = '今月の従星は' + mJyusei + 'で出力が絞られるところに、ぶつかりやすい配置も重なっています。予定を詰めず、引き受ける量を減らすほうが流れに合います。';
  } else if (mEnergy > 0 && mEnergy <= 2) {
    summary = '今月の従星は' + mJyusei + 'で、押し出す力は出にくい月です。無理に前へ出るより、整える・見直すほうに使うと収まりがいいです。';
  } else if (hasCaution && hasGood) {
    // 【260901修正】固定文だと「毎月同じ」に見える。mJyuseiで決定的にバリエーションを分ける。
    var mMixedVariants = [
      '今月は伸びる気配と気をつけたい気配が両方来ています。的を絞って動くと結果が出やすい月です。',
      '今月は' + mJyusei + 'で、良い流れと引っかかりやすい流れが同時に来ています。手を広げすぎず、優先度の一番高いものだけ進める月にすると噛み合います。',
      '今月は追い風と逆風が両方吹いています。' + mJyusei + 'の力を、広げる方向にだけ使うと結果が出やすいです。'
    ];
    summary = mMixedVariants[mEnergy % mMixedVariants.length];
  } else {
    // 【260901修正】ここも固定文だとフォールバック頻度がそのまま「毎月同じ」に見える。
    var mQuietVariants = [
      '今月はいくつかの気が静かに重なっています。上の各項目のうち、ご自身に響くものを手がかりに過ごしてみてください。',
      '今月は' + mJyusei + 'が巡る、これといった強い干渉のない月です。無理に意味づけず、上の各項目を手がかりにいつも通り過ごすので大丈夫です。',
      '今月は' + mJyusei + 'の力を含めて、特別に踏み込む必要のない月です。上の項目で気になるところだけ意識してみてください。'
    ];
    summary = mQuietVariants[mEnergy % mQuietVariants.length];
  }
...
// 従星ごとのエネルギー値（1〜12）。総合判定で「今日の勢い」を測るのに使う。
var JYUSEI_ENERGY = {
  '天報星':3, '天印星':6, '天貴星':9, '天恍星':7, '天南星':10, '天禄星':11,
  '天将星':12, '天堂星':8, '天胡星':4, '天極星':2, '天庫星':5, '天馳星':1
};
...
function getAspectKeys(aspectsText) {
  ...
  // または computeFortune() が返す ['年柱：大半会', '日柱：対冲'] のような配列。
  // 【260815修正】配列で渡された場合、配列の indexOf は要素の完全一致でしか
  //   ヒットしないため（'年柱：大半会' !== '大半会'）、位相が1つも検出されず
  //   hasGood / hasCaution が立たなかった。総合の一言が従星の点数だけで
  //   決まってしまう原因になっていたので、必ず文字列に正規化してから走査する。
  if (Array.isArray(aspectsText)) aspectsText = aspectsText.join(' ');
  aspectsText = String(aspectsText || '');
...
function buildTodayGuidance(todayF, stg, isTenchuDay) {
  ...
  } else if (energy > 0 && energy <= 2 && hasGood) {
    // 【260815修正】出力が底 × 合法（半会・大半会・支合・三合等）
    //   従星の点数だけで「休め」と結論を出していた実装の誤り。
    //   低出力は「自分から押す力が出ない」であって「何も起きない」ではない。
    //   そこに合法（まとまる・広がる方向）が同時に出ているなら、
    //   動きはむしろ起きる。ただし主導権が自分側にないだけ。
    //   → 正しい実務は「休め」ではなく「押すな、受けろ」。
    summary = '今日は' + jyusei + 'で、自分から押していく力は出にくい日です。ただ、まとまる方向の配置が同時に出ています。こちらから仕掛けるより、来た話・来た誘いを受けるほうが動きます。断らずに受け取っておくと、後でつながります。';
  } else if (energy > 0 && energy <= 2 && hasCaution) {
    // 出力が底 × 散法：ここは本当に守りの日
    summary = '今日は' + jyusei + 'で出力が絞られるところに、ぶつかりやすい配置も重なっています。予定を詰めず、引き受ける量を減らすほうが流れに合います。';
  } else if (energy > 0 && energy <= 2) {
    // 出力が底 × 位相なし：静かな日
    summary = '今日は' + jyusei + 'で、押し出す力は出にくい日です。無理に前へ出るより、整える・見直すほうに使うと収まりがいいです。';
  } else if (hasCaution && hasGood) {
    // 【260901修正】固定文1本だと「毎日同じ文」に見える（yui指摘）。
    //   jyusei（従星）で決定的にバリエーションを分ける。ランダムにはしない
    //   （同じ日に開き直した時に文言が変わると不信感を生む＝再現性は必須）。
    var mixedVariants = [
      '今日は伸びる気配と気をつけたい気配が両方重なっています。全部を進めようとせず、一つに絞って動くと結果が出やすい日です。',
      '今日は' + jyusei + 'で、良い流れと引っかかりやすい流れが同時に来ています。欲張らず、一番大事な一つだけ前に進める日にすると噛み合います。',
      '今日は追い風と逆風が両方吹いています。' + jyusei + 'の力を、広げる方向にだけ使うと結果が出やすいです。'
    ];
    summary = mixedVariants[energy % mixedVariants.length];
  } else {
    // 特別な干渉なし：断定せず、各軸を自分で読んでもらう。
    // 【260901修正】ここも固定文1本だとフォールバック頻度(月30〜50%)がそのまま
    //   「毎日同じ文」に見えてしまう。jyusei・stageNameで文面を変え、
    //   断定しない姿勢は保ったままバリエーションを持たせる。
    var quietVariants = [
      '今日はいくつかの気が静かに重なっています。上の各項目のうち、ご自身に響くものを手がかりに過ごしてみてください。',
      '今日は' + jyusei + 'が巡る、これといった強い干渉のない日です。無理に意味づけず、上の各項目を手がかりにいつも通り過ごすので大丈夫です。',
      '今日は「' + stageName + '」の局面。' + jyusei + 'の力も含めて、特別に踏み込む必要のない日です。上の項目で気になるところだけ意識してみてください。'
    ];
    summary = quietVariants[energy % quietVariants.length];
  }
